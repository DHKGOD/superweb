...
document.addEventListener('DOMContentLoaded', function () {
    ...
    get_button.addEventListener('click', function () {
        if(supportCount<5){
            ...
            var convert_dhd = Number(dhd)
            var dhd_plus = convert_dhd +quotient
            dhd_plus_conver = dhd_plus.toString();
            document.getElementById('info_bar').textContent = `等級: ${levlel} , 狗狗幣:  ${dgd} , 狗毛幣:  ${dhd_plus_conver}  `;
            document.getElementById('supportCount').textContent = `助力次數: ${reminder}`;
            supportCount = reminder
            localStorage.setItem('dhd',dhd_plus);
        }
